...
const StarRating = ({ rating = 0 }) => (
  <span style={{ color: '#EAB308', fontSize: '0.75rem', letterSpacing: '-0.05em' }}>
    {[1,2,3,4,5].map(i => (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem' }}>
        <div style={{
          background: '#18181B', borderRadius: '10px',
      ...
      {/* Title & meta */}
      <div>
        <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: '1rem', color: '#F4F4F5', lineHeight: 1.3, marginBottom: '0.4rem' }}>
          {upload.title}
        </div>
        <div style={{ fontSize: '0.78rem', color: '#52525B', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: '0.75rem', color: '#3F3F46', fontFamily: "'DM Mono', monospace" }}>
          ↓ {upload.downloadCount || 0}
        </span>
      </div>

      {/* Uploader */}
      <div style={{ fontSize: '0.78rem', color: '#3F3F46', borderTop: '1px solid #18181B', paddingTop: '0.75rem' }}>
        By <span style={{ color: '#52525B' }}>{upload.uploader?.name || 'Anonymous'}</span>
      </div>

      {/* Actions */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {isAdmin ? (
          // Admin gets free download — no payment needed
          <button
            onClick={() => onBuy(upload._id, 'download')}
            disabled={buying === upload._id}
            style={{
              padding: '0.7rem', borderRadius: '9px', border: 'none',
              background: '#18181B', color: '#EAB308',
          ...
          >
            {buying === upload._id ? 'Opening...' : '↓ Admin Download'}
          </button>
        ) : hasSubscription ? (
          <button
            onClick={() => onBuy(upload._id, 'download')}
...
const Marketplace = () => {
  ...
  const handleBuy = async (uploadId, mode) => {
    ...
    try {
      if (mode === 'download') {
        // Stream through backend — avoids Cloudinary auth issues
  ...
  return (
    <>
      <style>{`
        ...
        .mp-input:focus { border-color: #52525B; }
        .mp-input::placeholder { color: #3F3F46; }
        ...
        .mp-select:focus { border-color: #52525B; }
      ...
      <div className="mp-root">
        ...
        <div className="mp-inner">
          ...
          <div style={{ marginBottom: '2.5rem' }}>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '0.72rem', color: '#52525B', letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
          ...
          {user && !hasSubscription && (
            <div style={{
              background: 'linear-gradient(135deg, #18180A, #111108)',
              ...
              <button
                ...
                style={{
                  padding: '0.6rem 1.25rem', background: '#EAB308', color: '#09090B',
          ...
          {/* Grid */}
          {loading ? (
            <div style={{ textAlign: 'center', padding: '5rem', color: '#3F3F46' }}>
            ...
            <div style={{
              ...
              border: '1px dashed #27272A', borderRadius: '14px', color: '#3F3F46'
            }}>
              <div style={{ fontSize: '2.5rem', marginBottom: '1rem' }}>📭</div>
              <div style={{ fontWeight: 700, color: '#52525B', marginBottom: '0.4rem' }}>No documents found</div>
              <div style={{ fontSize: '0.85rem' }}>
                {hasFilters ? 'Try adjusting your filters.' : 'No approved documents yet. Check back soon.'}
              </div>
              {hasFilters && (
                <button onClick={clearFilters} style={{
                  marginTop: '1rem', background: 'none', border: '1px solid #27272A',
                  color: '#71717A', borderRadius: '8px', padding: '0.5rem 1.25rem',
